...
try:
    ...
    # Explicitly import all used constants for clarity if needed by other linters/tools
    ...
    class MockConstants:
        ...
        SUT_BT_VOLTAGE = 480; EPS_CURRENT_LIMIT = 2000; DUT_POWER_LIMIT = 1350
        CAPACITORS_BY_VOLTAGE = {}; Q_SWITCH_POWERS = {"generic_cp": []}
        CS_SWITCHES_BY_VOLTAGE_TRI = {}; CS_SWITCHES_BY_VOLTAGE_MONO = {}
        SUT_AT_MIN_VOLTAGE = 14000; SUT_AT_MAX_VOLTAGE = 140000; SUT_AT_STEP_VOLTAGE = 3000
    const = MockConstants()
    potencia_magnet_data, perdas_nucleo_data, potencia_magnet_data_H110_27, perdas_nucleo_data_H110_27 = {}, {}, {}, {}
    FATOR_CONSTRUCAO_PERDAS_H110_27, FATOR_CONSTRUCAO_POTENCIA_MAG_H110_27 = 1.15, 1.2
    FATOR_EXCITACAO_DEFAULT_TRIFASICO, FATOR_EXCITACAO_DEFAULT_MONOFASICO = 3, 5
    SUT_BT_VOLTAGE, EPS_CURRENT_LIMIT, DUT_POWER_LIMIT = 480, 2000, 1350
    CAPACITORS_BY_VOLTAGE, Q_SWITCH_POWERS = {}, {"generic_cp": []}
    CS_SWITCHES_BY_VOLTAGE_TRI, CS_SWITCHES_BY_VOLTAGE_MONO = {}, {}
    SUT_AT_MIN_VOLTAGE, SUT_AT_MAX_VOLTAGE, SUT_AT_STEP_VOLTAGE = 14000, 140000, 3000


log = logging.getLogger(__name__)
epsilon = const.EPSILON

# --- Pydantic Models (그대로 유지) ---
class NoLoadLossesInput(BaseModel):
    perdas_vazio_ui: float
    peso_nucleo_ui: float
    corrente_excitacao_ui: float
    inducao_ui: float
    corrente_exc_1_1_ui: Optional[float] = None
    corrente_exc_1_2_ui: Optional[float] = None
    frequencia: float
    tensao_bt_kv: float
    corrente_nominal_bt: float
...
class LoadLossesInput(BaseModel):
    ...
    impedancia: float
    tensao_at_kv: float
    tensao_at_tap_maior_kv: float
    tensao_at_tap_menor_kv: float
    impedancia_tap_maior: float
    impedancia_tap_menor: float
    corrente_nominal_at_a: float
    corrente_nominal_at_tap_maior_a: float
    corrente_nominal_at_tap_menor_a: float
    tipo_transformador: str = "Trifásico"
    perdas_vazio_kw_calculada: float


# --- Helper Functions (safe_float, interpolate_losses_table_data - 그대로 유지) ---
...
def interpolate_losses_table_data(table_name: str, induction: float, frequency: float, steel_type: str = "M4") -> float:
    ...
    if not table: # Check if table is empty
    ...
    if not inductions or not frequencies: # Check if lists are empty
    ...
    if abs(i0 - i1) < epsilon: # Linear interpolation for frequency
        v0, v1 = table.get((i0, f0), 0.0), table.get((i0, f1), 0.0)
        return v0 + (v1 - v0) * (frequency - f0) / (f1 - f0) if abs(f1 - f0) > epsilon else v0
    if abs(f0 - f1) < epsilon: # Linear interpolation for induction
...
# --- Capacitor Bank Calculation Helpers (그대로 유지) ---
...
def calculate_q_combination_power(q_combination, available_caps_for_voltage):
    total_power = 0
    power_steps = Q_SWITCH_POWERS.get("generic_cp")
    if not power_steps or len(power_steps) != 5: return 0
...
def get_cs_configuration(target_bank_voltage_key: Optional[str], use_group1_only: bool, circuit_type: str) -> str:
    ...
    if not available_switches: return f"N/A (Sem CS p/ {target_bank_voltage_key}kV)"
    
    cs_config_list = []
    for switch_name in available_switches:
        is_group_2_switch = len(switch_name) > 4 and switch_name.endswith("2")
        if use_group1_only and circuit_type == "Trifásico" and is_group_2_switch: continue
...
def find_best_q_configuration(target_bank_voltage_key: Optional[str], required_power_mvar: float, use_group1_only: bool) -> tuple[str, float]:
    if target_bank_voltage_key is None or required_power_mvar <= epsilon: return "N/A", 0.0
    
    all_available_caps = CAPACITORS_BY_VOLTAGE.get(target_bank_voltage_key, [])
    if not all_available_caps: return f"N/A (Sem Caps p/ {target_bank_voltage_key}kV)", 0.0

    available_caps = [cap for cap in all_available_caps if len(cap) > 4 and cap.endswith("1")] if use_group1_only else all_available_caps
    if not available_caps and use_group1_only: available_caps = all_available_caps

    if not available_caps: return f"N/A (Sem Caps Sel. p/ {target_bank_voltage_key}kV)", 0.0
...
# --- No-Load Losses Calculation (그대로 유지) ---
def calculate_no_load_losses(data_in: Dict[str, Any]) -> Dict[str, Any]:
    ...
    corrente_excitacao_calc_a = potencia_mag_aco_kvar / den_corr_exc_calc if den_corr_exc_calc > epsilon else 0
    corrente_excitacao_percentual_calc = (corrente_excitacao_calc_a / data.corrente_nominal_bt) * 100 if data.corrente_nominal_bt > epsilon else 0
    ...
    fator_perdas_projeto_kw_ton = data.perdas_vazio_ui / data.peso_nucleo_ui if data.peso_nucleo_ui > epsilon else 0
    fator_perdas_projeto_w_kg = fator_perdas_projeto_kw_ton
    ...
    fator_pot_mag_projeto_var_kg = (potencia_mag_projeto_kvar * 1000) / peso_nucleo_kg_ui if peso_nucleo_kg_ui > epsilon else 0
    ...
    corrente_exc_1_1_proj_a = (data.corrente_nominal_bt * (data.corrente_exc_1_1_ui / 100.0)) if data.corrente_exc_1_1_ui is not None else (fator_exc_default * corrente_excitacao_projeto_a)
    corrente_exc_1_2_proj_a = (data.corrente_nominal_bt * (data.corrente_exc_1_2_ui / 100.0)) if data.corrente_exc_1_2_ui is not None else None
    ...
    scenarios_no_load = {
        ...
        "1.2pu": {"Vtest_kv": tensao_teste_1_2_kv, "Itest_a": corrente_exc_1_2_proj_a}
    }
    ...
    results = {
        "calculos_baseados_aco": {
            ...
            "perdas_vazio_kw": round(data.perdas_vazio_ui, 2) # Adicionando a perda em vazio de entrada aqui para o LoadLoss ter acesso.
        },
        "calculos_vazio": { 
            "tensao_teste_1_1_kv": round(tensao_teste_1_1_kv, 2),
            "tensao_teste_1_2_kv": round(tensao_teste_1_2_kv, 2),
            "corrente_excitacao_1_1_calc_a": round(corrente_exc_1_1_proj_a, 2),
...
# --- SUT/EPS Compensated Current (그대로 유지) ---
def calculate_sut_eps_current_compensated(
    ...
    results_comp = {"corrente_eps_sf_a": I_dut_reflected_amps, "percent_limite_sf": (I_dut_reflected_amps/EPS_CURRENT_LIMIT)*100 if EPS_CURRENT_LIMIT > 0 else float('inf'),
                 "corrente_eps_cf_a": I_dut_reflected_amps, "percent_limite_cf": (I_dut_reflected_amps/EPS_CURRENT_LIMIT)*100 if EPS_CURRENT_LIMIT > 0 else float('inf')}
...
# --- Status String Generation Helper (similar to old CapBankStatusAnalyzer) ---
def get_scenario_status_string(test_v, bank_v_cf, power_cf_req, bank_v_sf, power_sf_req, corrente, potencia_ativa):
    """Generates a status string based on various limits."""
    status_parts = []
    events = 0
    
    # Validate inputs (simplified)
    test_v = safe_float(test_v, 0)
    bank_v_cf = safe_float(bank_v_cf, 0)
    power_cf_req = safe_float(power_cf_req, 0)
    bank_v_sf = safe_float(bank_v_sf, 0) # Can be 0 if not applicable
    power_sf_req = safe_float(power_sf_req, 0) # Can be 0
    corrente = safe_float(corrente, 0)
    potencia_ativa = safe_float(potencia_ativa, 0)

    # 1. Voltage Check (C/F bank)
    if bank_v_cf > epsilon:
        limit_v = bank_v_cf * 1.1
        if test_v > limit_v + epsilon:
            percent_above = ((test_v / limit_v) - 1) * 100 if limit_v > epsilon else float('inf')
            status_parts.append(f"(V) > Limite ({percent_above:.0f}%)")
            events += 1
    
    # 2. Current Check
    if corrente > EPS_CURRENT_LIMIT + epsilon: # Use EPS_CURRENT_LIMIT from const
        status_parts.append(f"(A) > Limite ({corrente:.0f}A)")
        events += 1

    # 3. Active Power Check (DUT_POWER_LIMIT for kW)
    if potencia_ativa > DUT_POWER_LIMIT + epsilon: # Use DUT_POWER_LIMIT from const
        status_parts.append(f"(P) > Limite ({potencia_ativa:.0f}kW)")
        events += 1

    # 4. Reactive Power Check (using required power for C/F as primary, then S/F)
    # Thresholds from old ParameterAnalyzer logic (pteste_high, pteste_medium)
    # These should probably be constants too. For now, hardcoding.
    potencia_critica_mvar = 93.6 
    potencia_alerta_mvar = 46.8

    power_to_check = power_cf_req if power_cf_req > epsilon else power_sf_req # Prioritize C/F

    if power_to_check > potencia_critica_mvar + epsilon:
        status_parts.append(f"CapBank Q_req ↑ ({potencia_critica_mvar:.0f}+ MVAr)")
        events +=1
    elif power_to_check > potencia_alerta_mvar + epsilon:
        status_parts.append(f"CapBank Q_req ↑ ({potencia_alerta_mvar:.0f}+ MVAr)")
        events +=1

    if not status_parts: return "OK"
    if events > 1: status_parts.append(f"({events})") # Indicar múltiplos alertas
...
def calculate_load_losses(data_in: Dict[str, Any]) -> Dict[str, Any]:
    ...
    temp_factor = (235.0 + 25.0) / (235.0 + data.temperatura_referencia)
    perdas_pu_nominal = (perdas_carga_sem_vazio_nom / (data.potencia_mva * 1000.0)) * 100 if data.potencia_mva > epsilon else 0
    resistencia_percentual = perdas_pu_nominal
    reatancia_percentual = math.sqrt(max(0, data.impedancia**2 - resistencia_percentual**2))
    fp_curto_circuito = resistencia_percentual / data.impedancia if data.impedancia > epsilon else 0
    i2r_perdas_nom = perdas_carga_sem_vazio_nom * 0.85 
    ...
    condicoes_nominais_results = {
        ...
        "perdas_totais_tap_menos": round(data.perdas_carga_kw_u_min, 2),
        "perdas_totais_tap_mais": round(data.perdas_carga_kw_u_max, 2),
        "i2r_perdas": round(i2r_perdas_nom, 2),
        "perdas_adicionais": round(perdas_adicionais_nom, 2),
        "perdas_vazio_kw": round(data.perdas_vazio_kw_calculada, 2)
    }
    ...
    max_overall_q_mvar_req = 0.0
    
    test_types = ["25°C", "Frio", "Quente"]
    if data.tensao_at_kv >= 230: 
        test_types.extend(["1.2 pu", "1.4 pu"])

    for tap_label, Vnom_kv_tap, Inom_a_tap, Z_percent_tap, Pcarga_total_kw_tap in cenarios_config_list:
        current_tap_data = {
            "nome_tap": tap_label,
            "test_params_tap": { # Basic V, I, Z for this specific tap
                "tensao_kv": Vnom_kv_tap, "corrente_a": Inom_a_tap, "vcc_percent": Z_percent_tap,
                "vcc_kv": (Vnom_kv_tap / 100.0) * Z_percent_tap
            },
            "cenarios_do_tap": []
        }
        ...
        if Pcarga_sem_vazio_kw_tap <= epsilon:
            ...
            all_taps_data.append(current_tap_data) # Add tap with empty scenarios
            continue
        
        Pcc_frio_kw_tap = Pcarga_sem_vazio_kw_tap * temp_factor
        Vcc_kv_tap = current_tap_data["test_params_tap"]["vcc_kv"]


        for test_type_label in test_types:
            scen_res_dict: Dict[str, Any] = {"nome_cenario_teste": test_type_label}
            Vtest_kv_scen, Itest_a_scen, Pativa_kw_scen = 0,0,0
            
            if test_type_label == "25°C":
                Vtest_kv_scen, Itest_a_scen, Pativa_kw_scen = Vcc_kv_tap, Inom_a_tap, Pcc_frio_kw_tap
            elif test_type_label == "Frio":
                ratio_sqrt_frio = math.sqrt(Pcarga_total_kw_tap / Pcc_frio_kw_tap) if Pcc_frio_kw_tap > epsilon else 0
                ...
                ratio_sqrt_quente = math.sqrt(Pcarga_sem_vazio_kw_tap / Pcc_frio_kw_tap) if Pcc_frio_kw_tap > epsilon else 0
                ...
                Pativa_kw_scen = Pcarga_sem_vazio_kw_tap * (pu_factor**2)

            Pteste_kva_scen = Vtest_kv_scen * Itest_a_scen * sqrt_3_factor
            Pteste_mva_scen = Pteste_kva_scen / 1000.0
            Pteste_mvar_req_scen = math.sqrt(max(0, Pteste_kva_scen**2 - Pativa_kw_scen**2)) / 1000.0 if Pteste_kva_scen >= Pativa_kw_scen else 0
            ...
            bank_cf_details = {"tensao_disp_kv": None, "q_req_mvar": Pteste_mvar_req_scen, "q_provided_mvar": 0.0, "cs_config": "N/A", "q_config": "N/A"}
            if target_v_cf_key and Pteste_mvar_req_scen > epsilon:
                ...
                max_p_g1_cf = calculate_q_combination_power([1,2,3,4,5], g1_caps_cf) if g1_caps_cf else 0
                ...
                q_cfg, q_prov = find_best_q_configuration(target_v_cf_key, Pteste_mvar_req_scen, use_g1_cf)
                bank_cf_details["q_config"], bank_cf_details["q_provided_mvar"] = q_cfg, q_prov
            scen_res_dict["cap_bank_cf"] = bank_cf_details
            
            bank_sf_details = {"tensao_disp_kv": None, "q_req_mvar": Pteste_mvar_req_scen, "q_provided_mvar": 0.0, "cs_config": "N/A", "q_config": "N/A"}
            if target_v_sf_key and Pteste_mvar_req_scen > epsilon:
                ...
                max_p_g1_sf = calculate_q_combination_power([1,2,3,4,5], g1_caps_sf) if g1_caps_sf else 0
                ...
                q_cfg, q_prov = find_best_q_configuration(target_v_sf_key, Pteste_mvar_req_scen, use_g1_sf)
                bank_sf_details["q_config"], bank_sf_details["q_provided_mvar"] = q_cfg, q_prov
            scen_res_dict["cap_bank_sf"] = bank_sf_details

            scen_res_dict["status_cenario"] = get_scenario_status_string(
                Vtest_kv_scen, bank_cf_details["tensao_disp_kv"], bank_cf_details["q_req_mvar"],
                bank_sf_details["tensao_disp_kv"], bank_sf_details["q_req_mvar"],
            ...
            if taps_sut_hv_v_iter.any():
                ...
                top_taps_v_sut = sorted(taps_sut_hv_v_iter, key=lambda tap_v_sut: diffs_sut[tap_v_sut])[:5]
